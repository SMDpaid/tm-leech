...
class DbManger:
    ...
    def connect(self):
        try:
            ...
            LOGGER(__name__).error(f"Error in DB connection: {error}")
            self.err = True
    ...
    def db_init(self):
        ...
        self.cur.execute(sql)
        self.conn.commit()
        LOGGER(__name__).info("Database Initiated")
        self.db_load()
    ...
    def db_load(self):
            ...
            for row in rows:
                ...
                if row[5] is not None and not ospath.exists(path):
                    ...
                    with open(path, 'wb+') as f:
                        f.write(row[5])
                        f.close()
            LOGGER(__name__).info("Users data has been imported from Database")
        # Rss Data
        ...
        if rows:
            ...
            LOGGER(__name__).info("Rss data has been imported from Database.")
        self.disconnect()
