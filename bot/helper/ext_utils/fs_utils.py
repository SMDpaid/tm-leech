...
def exit_clean_up(signal, frame):
    try:
        LOGGER(__name__).info("Please wait, while we clean up the downloads and stop running downloads")
        clean_all()
        sysexit(0)
    except KeyboardInterrupt:
        LOGGER(__name__).warning("Force Exiting before the cleanup finishes!")
        sysexit(1)

def clean_unwanted(path: str):
    LOGGER(__name__).info(f"Cleaning unwanted files/folder: {path}")
...
def get_media_info(path):
    try:
        ...
        LOGGER(__name__).error(f"get_media_info: {e}")
        return 0, None, None
    try:
...
def get_video_resolution(path):
    try:
        ...
        LOGGER(__name__).error(f"get_video_resolution: {e}")
        return 480, 320
