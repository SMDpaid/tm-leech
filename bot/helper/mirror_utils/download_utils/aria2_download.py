...
def __onDownloadStarted(api, gid):
    if STOP_DUPLICATE or TORRENT_DIRECT_LIMIT is not None or ZIP_UNZIP_LIMIT is not None:
        ...
        try:
            ...
            LOGGER(__name__).error(f"onDownloadStart: {gid} stop duplicate and size check didn't pass")

@new_thread
def __onDownloadComplete(api, gid):
    LOGGER(__name__).info(f"onDownloadComplete: {gid}")
    dl = getDownloadByGid(gid)
    download = api.get_download(gid)
    if download.followed_by_ids:
        ...
        LOGGER(__name__).info(f'Changed gid from {gid} to {new_gid}')
    elif dl:
...
def __onDownloadError(api, gid):
    LOGGER(__name__).info(f"onDownloadError: {gid}")
    sleep(0.5)
    dl = getDownloadByGid(gid)
    try:
        download = api.get_download(gid)
        error = download.error_message
        LOGGER(__name__).info(f"Download Error: {error}")
    except:
        pass
    if dl:
...
def add_aria2c_download(link: str, path, listener, filename):
    ...
    if download.error_message:
        ...
        LOGGER(__name__).info(f"Download Error: {error}")
        return sendMessage(error, listener.bot, listener.update, chat_id=chat_id)
    with download_dict_lock:
        download_dict[listener.uid] = AriaDownloadStatus(download.gid, listener)
        LOGGER(__name__).info(f"Started: {download.gid} DIR: {download.dir} ")
