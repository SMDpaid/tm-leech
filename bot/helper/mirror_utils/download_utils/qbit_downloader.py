...
def add_qb_torrent(link, path, listener, select):
    ...
    msg = listener.update.message if listener.update.message is not None else listener.update.channel_post
    chat_id = msg.chat.id
    ...
    try:
        ...
        if len(tor_info) > 0:
            sendMessage("This Torrent is already in list.", listener.bot, listener.update, chat_id=chat_id)
            client.auth_log_out()
            return
        if is_file:
        ...
        if op.lower() == "ok.":
            ...
            if len(tor_info) == 0:
                while True:
                    if time() - meta_time >= 30:
                        ...
                        sendMessage(ermsg, listener.bot, listener.update, chat_id=chat_id)
            ...
            sendMessage("This is an unsupported/invalid link.", listener.bot, listener.update, chat_id=chat_id)
            client.torrents_delete(torrent_hashes=ext_hash, delete_files=True)
            client.auth_log_out()
            return
        tor_info = tor_info[0]
        ...
        LOGGER(__name__).info(f"QbitDownload started: {tor_info.name} - Hash: {ext_hash}")
        ...
        if BASE_URL is not None and select:
            if not is_file:
                ...
                meta = sendMessage(metamsg, listener.bot, listener.update, chat_id=chat_id)
                while True:
                    tor_info = client.torrents_info(torrent_hashes=ext_hash)
                    if len(tor_info) == 0:
                        deleteMessage(listener.bot, meta, chat_id=chat_id)
                        return
                    try:
                        tor_info = tor_info[0]
            ...
            sendMarkup(msg, listener.bot, listener.update, QBBUTTONS, chat_id=chat_id)
        else:
            sendStatusMessage(listener.update, listener.bot, chat_id=chat_id)
    except Exception as e:
        sendMessage(str(e), listener.bot, listener.update, chat_id=chat_id)
        client.auth_log_out()
    ...
    while True:
        ...
        try:
            ...
            if tor_info.state == "metaDL":
                stalled_time = time()
                if time() - meta_time >= 900: # timeout while downloading metadata
                    client.torrents_pause(torrent_hashes=ext_hash)
                    sleep(0.3)
                ...
                if STOP_DUPLICATE and not listener.isLeech and not dupChecked and ospath.isdir(f'{path}'):
                    LOGGER(__name__).info('Checking File/Folder if already in Drive')
                    ...
                    if not listener.extract:
                        ...
                        if qbmsg:
                            ...
                            sendMarkup("Here are the search results:", listener.bot, listener.update, button, chat_id=chat_id)
                            client.torrents_delete(torrent_hashes=ext_hash, delete_files=True)
                            client.auth_log_out()
                            break
                    dupChecked = True
                if not sizeChecked:
                    ...
                    if limit is not None:
                        LOGGER(__name__).info('Checking File/Folder Size...')
                        sleep(1)
                        size = tor_info.size
                ...
                if not rechecked and 0.99989999999999999 < tor_info.progress < 1:
                    LOGGER(__name__).info(f"Force recheck - Name: {tor_info.name} Hash: {ext_hash} Downloaded Bytes: {tor_info.downloaded} Size: {tor_info.size} Total Size: {tor_info.total_size}")
                    client.torrents_recheck(torrent_hashes=ext_hash)
                    rechecked = True
                elif time() - stalled_time >= 43200: # timeout after downloading metadata
                    client.torrents_pause(torrent_hashes=ext_hash)
                    sleep(0.3)
                    listener.onDownloadError("Dead Torrent!")
                    client.torrents_delete(torrent_hashes=ext_hash, delete_files=True)
                ...
                LOGGER(__name__).info(f"onQbDownloadComplete: {ext_hash}")
                uploaded = True
                if not QB_SEED:
                    client.torrents_pause(torrent_hashes=ext_hash)
                if select:
                ...
                if QB_SEED:
                    with download_dict_lock:
                        ...
                        download_dict[listener.uid] = QbDownloadStatus(listener, client, gid, ext_hash, select)
                    update_all_messages()
                    LOGGER(__name__).info(f"Seeding started: {tor_info.name}")
                else:
                    client.torrents_delete(torrent_hashes=ext_hash, delete_files=True)
                    client.auth_log_out()
                    break
                ...
                Thread(target=auto_delete, args=(listener.bot, listener.update.message, auto)).start()
                client.torrents_delete(torrent_hashes=ext_hash, delete_files=True)
                client.auth_log_out()
                break
        except Exception as e:
...
def get_confirm(update, context):
    ...
    if qbdl is None:
        ...
        msg = qbdl.listener().update.message if qbdl.listener().update.message is not None else qbdl.listener().update.channel_post
        chat_id = msg.chat.id
        
        sendStatusMessage(qbdl.listener().update, qbdl.listener().bot, chat_id=chat_id)
        query.message.delete()
