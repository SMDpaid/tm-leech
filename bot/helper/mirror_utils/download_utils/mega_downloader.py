...
class MegaAppListener(MegaListener):
    ...
    def onRequestFinish(self, api, request, error):
        ...
        if request_type == MegaRequest.TYPE_LOGIN:
            ...
            LOGGER(__name__).info("Fetching Root Node.")
            self.node = api.getRootNode()
            LOGGER(__name__).info(f"Node Name: {self.node.getName()}")
        if request_type not in self._NO_EVENT_ON or self.node and "cloud drive" not in self.node.getName().lower():
            self.continue_event.set()

    def onRequestTemporaryError(self, api, request, error: MegaError):
        LOGGER(__name__).error(f'Mega Request error in {error}')
    ...
    def onTransferTemporaryError(self, api, transfer, error):
        ...
        LOGGER(__name__).error(f'Mega download error in file {transfer} {filen}: {error}')
        if state in [1, 4]:
...
def add_mega_download(mega_link: str, path: str, listener):
    ...
    if link_type == "file":
        LOGGER(__name__).info("File. If your download didn't start, then check your link if it's available to download")
        executor.do(api.getPublicNode, (mega_link,))
        node = mega_listener.public_node
    else:
        LOGGER(__name__).info("Folder. If your download didn't start, then check your link if it's available to download")
