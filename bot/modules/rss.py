...
def rss_sub(update, context):
    try:
        ...
        if exists is not None:
            LOGGER(__name__).error("This title already subscribed! Choose another title!")
        ...
        try:
            ...
            LOGGER(__name__).info(f"Rss Feed Added: {title} - {feed_link} - {filters}")
...
def rss_unsub(update, context):
    try:
        ...
        if exists is None:
            ...
            LOGGER(__name__).info(f"Rss link with Title: {title} has been removed!")
    except IndexError:
...
def rss_unsuball(update, context):
    if len(rss_dict) > 0:
        ...
        LOGGER(__name__).info("All Rss Subscriptions has been removed")
    else:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    LOGGER(__name__).info(f"Reached Max index no. {feed_count} for this feed: {name}. \
            ...
            LOGGER(__name__).info(f"Feed Name: {name}")
            LOGGER(__name__).info(f"Last item: {last_link}")
        except Exception as e:
            LOGGER(__name__).error(f"{e} Feed Name: {name} - Feed Link: {data[0]}")
            continue
